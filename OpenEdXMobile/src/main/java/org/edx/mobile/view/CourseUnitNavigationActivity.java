...
public class CourseUnitNavigationActivity extends CourseBaseActivity implements
    ...
    @InjectView(R.id.lyt_prev)
    private LinearLayout prevLyt;
    @InjectView(R.id.lyt_next)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prevLyt.setOnClickListener(view -> navigatePreviousComponent());
        nextLyt.setOnClickListener(view -> navigateNextComponent());
    }
    ...
    public void navigateNextComponent() {
        ...
        if (index < pagerAdapter.getCount() - 1) {
            ...
            loadBlockData();
         /*   onBackPressed();
            EventBus.getDefault().postSticky(new EventCourseDetail(""));*/
        }
    }
    ...
    public void loadBlockData() {
        ...
        getHierarchyCall = courseApi.getCourseStructureWithoutStale(config.getApiUrlVersionConfig().getBlocksApiVersion(),courseId);
        getHierarchyCall.enqueue(new CourseAPI.GetCourseStructureCallback(CourseUnitNavigationActivity.this, courseId,
                null, null, null, this) {
            @Override
            protected void onResponse(@NonNull final CourseComponent courseComponent) {
                ...
                selectedUnit = courseManager.getComponentById(config.getApiUrlVersionConfig().getBlocksApiVersion(),courseData.getCourse().getId(),
                        courseComponentId);
                nextContent();
            }
            ...
            protected void onFailure(@NonNull Throwable error) {
                ...
                if (error instanceof HttpStatusException) {
                    if (((HttpStatusException) error).getStatusCode() == 401) {
                        Toast.makeText(CourseUnitNavigationActivity.this, R.string.msg_logout_auth, Toast.LENGTH_SHORT).show();
                        environment.getRouter().performManualLogout(CourseUnitNavigationActivity.this,
    ...
    private void tryToUpdateForEndOfSequential() {
        ...
        if (curIndex + 1 <= pagerAdapter.getCount() - 1) {
            ...
            if (currentSubsectionId.equalsIgnoreCase(nextUnitSubsectionId)) {
                mNextUnitLbl.setVisibility(View.INVISIBLE);
            } else {
        ...
        if (curIndex - 1 >= 0) {
            ...
            if (currentSubsectionId.equalsIgnoreCase(prevUnitSubsectionId)) {
                mPreviousUnitLbl.setVisibility(View.INVISIBLE);
            } else {
